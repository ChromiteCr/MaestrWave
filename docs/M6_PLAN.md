...
| 4. 练习曲端点 | ✅ 已完成（M7e） | **改用符号乐谱，不调 text2music**，见下方偏离说明 |
| 5. 录制层 + 评分 | ✅ 已完成（M6f + M7e） | 用节拍器当拍网格；「力度对应」在 M7e 随练习曲接上 |
...
**与计划的偏离（第 4 步）**：计划写的是「调天琴 `text2music` 生成练习曲 + 对音频做能量
起始点检测找拍网格相位 + 置信度低时让用户跟拍手动校准」。实际做法是 M7 的符号乐谱那条链路：
`backend/practice.py` 直接**写谱**（音符、和弦、每小节的力度全是我们自己排的），再交给
`render.py` 渲染、混成一条音轨。

这一换把计划里三项工作直接消掉了：

1. **起始点检测不需要了。** 拍网格是写下的，不是测出来的 —— 第一小节第一拍精确落在
   数拍结束那一刻。「手动校准」那个兜底入口也就没有存在理由。
2. **`build_tags` 的两条扩展**（含打击乐、开头一小节数拍）不需要了。它们原本是对模型的
   **请求**，模型给不给是另一回事；现在是事实。
3. **「按 (lesson_id, bpm, meter) 缓存」变成了自然结果**：spec 的哈希就是 `piece_id`，
   同一份 spec 永远渲染出同一份字节，缓存命中与否只是快慢的区别。

顺带解决了两件计划里没提的事：**考试曲目不用往仓库里塞音频文件了**（`exam.ts` 里那三个
`audio: null` 就是在等这个）—— 「所有人考同一首」由「规格写死 + 渲染可复现」保证；
以及**「力度对应」终于评得了**：每小节的目标力度是谱面上写下的数，不是从音频测的
响度近似值。

代价是曲子是规则写出来的，不如生成模型那样有音乐性。对一首用来数拍子的练习曲来说，
拍子清楚、速度准确、能立刻拿到，比好听重要。

**还欠的**：全曲只支持一个速度，所以三级考试曲目原本要考的**渐慢没做**（要动
`score.py` 的时间换算与 `midi_out` 的 set_tempo，那是项目那条链路的公共代码）。
`exam.ts` 里已经把这一项从曲目描述和 `covers` 里去掉了，而不是留着一首「说是渐慢
其实匀速」的曲子。
...
- ~~`audio` 字段显式留成 `null` 表示「曲目未就绪」~~ —— M7e 起改成 `music` 规格，
  曲子由后端照规格写谱渲染。「固定」由规格是常量 + 渲染可复现保证，不再需要音频文件。
...
**已补齐（M7e）**：曲目走符号乐谱写谱渲染（见上文第 4 步的偏离说明），
「开始考试」真的能点了：选曲即后台渲染 → 自检 → 开考 → 打满小节自动出分 →
按 70/85 判及格与优秀。渐慢那一项没做，理由同上。
