 ...
 * ## 代价：确认是滞后的
 *
 * 一个角要等**出边**长到够了才能确认，因此拍点是回头才认出来的，实测滞后约
 * 176ms。给评分用完全没问题（评分本来就是录完再算，而且时间戳记的是
...
export interface IctusOptions {
   ...
   * 按拍长的 60% 设不应期之后回到 99%。见 `ConductingModel.setBaseBpm` ——
   * 那里还写了为什么这个比例**不能再往上加**。
   */
  ...
  /**
   * 「低角」判据的松紧：拍点要比两侧顶点的**平均高度**再低 `lowMarginRatio ×
   * 对角线` 才算数。正数更严，负数更松（允许拍点略高于那个平均值）。
   *
   * 扫下来这条轴在 −0.08~0 之间是**平的**（查全差 0.2pp，噪声量级），再严就开始
   * 掉（+0.05 时极端条件掉到 94%）。取 0 —— 「低于两侧的中点」本身就是判据，
   * 不需要再配一个魔数。别指望在这里调出什么来。
   */
...
export const DEFAULT_ICTUS_OPTIONS: IctusOptions = {
  /*
   * 在合成的真人轨迹上扫出来的（2/3/4 拍 × 60/88/120 BPM × 三个随机种子），
   * 而且**不只扫定参时那几档理想条件** —— 真人不是照着标尺挥的，所以另列一组
   * 极端条件：拍型只占画面 0.2、抖动 σ=0.016、反弹压到 35%、15fps。
   *
   *              常态 查全/查准/偏差       极端 查全/查准/偏差
   *   M7k2 之前   95.6% / 96.9% / 19ms    89.0% / 86.7% / 40ms
   *   现在        97.1% / 99.6% / 17ms    96.1% / 99.1% / 26ms
   *
   * 两件值得记下来的事：
   *
   * 1. `epsilonRatio` 提到 0.18 之后，`minEdgeRatio`（≤0.18 的范围内）与
   *    `minTurnDeg` 就**完全不起作用了** —— 0.10 和 0.18、20° 和 50°，几组数字
   *    一模一样。简化得够狠时活下来的顶点本来就边长够、转角够。它们现在只是给
   *    病态输入兜底，在这两个数上调参不会有任何效果。
   * 2. 代价是**确认更晚**：122ms → 176ms（边要更长才活得下来）。拍点的**时刻**
   *    没有变差（偏差 19ms → 17ms，记的一直是拐角自己的时间戳），变晚的只有
   *    「画面上什么时候亮起来」。见 `ConductingModel.acceptIctus`。
   */
  epsilonRatio: 0.18,
  minEdgeRatio: 0.14,
  minTurnDeg: 35,
...
export function detectIctus(
  ...
  for (let i = 1; i < poly.length - 1; i += 1) {
     ...
     *
     * 比的是**两侧的平均高度**，不是「比两侧都低」。后者是个零余量的严格不等式，
     * 相邻那个顶点只要被抖动推低一丁点，一个真拍点就整个被丢掉；简化阈值提到
     * 0.18 之后更糟 —— 简化会吃掉一些反弹顶点，于是拍点的邻居常常是**另一个
     * 拍点**，高度本就相当，严格判据当场失效。实测把这一条换回严格式，
     * 常态查全从 97% 掉到 84%。
     */
    if (opt.requireLowCorner
      && b.y >= (a.y + c.y) / 2 - span * opt.lowMarginRatio) continue;
