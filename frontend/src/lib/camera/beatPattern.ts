...
import { motionPattern } from "../teaching/motionTemplate";
import { patternPointAt, type BeatPattern, type Meter, type Point } from "../teaching/patterns";
...
export function templateShape(meter: Meter): Point[] {
  ...
  if (hit) return hit;
  // 用动作模板而不是图式：图式把末拍画在高处（那一拍同时是下一小节的预备），
  // 拿它当模板，一个拍点老老实实落在平面上的人反而会被判形状不对 —— 实测三拍
  // 丢 15 分、四拍丢 24 分。见 teaching/motionTemplate.ts
