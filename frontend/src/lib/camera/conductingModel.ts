...
export class ConductingModel {
  ...
  setBaseBpm(bpm: number): void {
     ...
     * （66 BPM 实测查准率 66%）。
     *
     * 60% 而不是原来的 45%：**这个数同时买到查准和查全**。一个误报落在真拍点
     * 前面时，不应期会顺手把紧跟着的那个真拍点也吞掉 —— 一次误报制造两处错。
     * 所以压掉误报本身就在救漏检：45% → 60%，常态查全 95.7% → 96.7%、
     * 查准 96.9% → 99.1%（配合 `epsilonRatio` 0.18，见 `ictusDetector.ts`）。
     *
     * **上限卡在这里，别再往上加。** 这个数是拿曲子的 BPM 算的，不是用户实际
     * 挥的速度，用户快到 1/r 倍就开始丢真拍：70% 时抢拍 25~40% 的人查全率
     * 直接掉到 80%，而 60% 还有 98.8%。
     */
    if (bpm > 0) this.ictus.setOptions({ minIntervalMs: Math.max(200, (60000 / bpm) * 0.6) });
  }
   ...
   *
   * 之所以要区分：多边形拐角要等出边长够了才敢认，实测滞后约 176ms（M7k2 把
   * 简化阈值提上去之后，从 122ms 变长了 —— 边要更长才活得下来。这是那次换来
   * 查全 89% → 97% 的代价，而它只影响**看起来什么时候亮**，不影响拍点的时刻）。评分读的是
