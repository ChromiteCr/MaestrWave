...
export const DIMENSIONS: Record<RubricDimension, { label: string; how: string; basis: string }> = {
  ictusTiming: {
    label: "拍点准确度",
    how: "先把你每一拍的落点和乐曲的拍网格配对，再看它们散得有多开。散度是围绕你自己的平均位置算的，整体偏早或偏晚单独扣分，而且留了一段免罚区。没对上的拍会按比例折算。",
    basis: "拍点是「这一拍落在哪一刻」的确切位置，乐手全靠它预判下一下什么时候来。散度大意味着每一拍他们都得重新猜，乐队自然跟不住。整体偏移又大又整齐的时候多半是耳机延迟，系统会认出这个特征并提醒你去校准，不会算在你头上。",
  },
  tempoStability: {
    label: "速度稳定性",
    how: "同时看两条，相邻拍间隔的抖动，以及后半段相对前半段的整体漂移，取更差的那一个。系统还会比对你的平均拍长和乐曲的实际速度，打成半速或双倍拍会另外扣分。",
    basis: "打拍速度本身就是你给乐队的速度指令。不该变的时候变了，乐队会跟着你一起飘。越打越快是最常见的一种，通常因为反弹越抬越小，手来不及走完就急着落下。",
  },
  patternShape: {
    label: "拍型准确度",
    how: "把你每一小节的轨迹和该拍号的标准图形拍型做 DTW 对齐，取平均距离。DTW 允许时间上的伸缩，所以打慢一点不会因此扣分，扣的是走向和形状。",
    basis: "图形拍型是乐手辨认「现在是第几拍」的唯一视觉线索。四拍的第 2 拍往右打是最常见的错误，它应该去你自己的左边。方向错了，乐队会从错的位置往下数。",
  },
  ictusClarity: {
    label: "拍点清晰度",
    how: "量拍点那一刻的手速是全程平均手速的几倍。标准拍型自己也有一个倍数，达到它就算满分。",
    basis: "拍点要由一个明确的加速和制动给出。匀速划圈的手也一直在动，可乐手看不出哪一下是拍。含糊的拍点等于没有拍点。落下的最后一小段要再加速一点，让手砸到平面上。",
  },
  planeConsistency: {
    label: "平面一致性",
    how: "取所有拍点的高度算标准差，再除以整个拍型的高度。看的是相对比例，所以拍型打大打小不影响这一项。",
    basis: "拍点应该落在同一个想象中的水平面（conducting plane）上，也就是站姿那一课找到的那个高度。示范图上拍点一拍比一拍高，那只是为了让几条弧线不互相压住，真手要落回同一个平面。",
  },
  dynamicsMatch: {
    label: "力度对应",
    how: "逐小节量你拍型的大小，和谱面写下的力度算相关性。这一首力度全程不变的话，就反过来看你的拍型有没有乱变。",
    basis: "力度由拍型的大小表达，高度和宽度一起变大就是渐强。只变高不变宽，看起来更像速度变了。乐曲没有力度变化时拍型却在忽大忽小，乐队会以为你在要求渐强渐弱。",
  },
  handIndependence: {
    label: "双手独立性",
    how: "打拍手维持拍型不停的同时，表情手能不能独立做自己的事。这一项目前还没有自动评分，它要求同时看懂两只手在做什么，现在的信号给不出这个判断。",
    basis: "两只手同时表达同一件事，等于把一半的表达能力扔掉了。镜像的表情手不提供任何新信息，还会跟拍型抢乐手的注意力。",
  },
  cueAccuracy: {
    label: "Cue 准确性",
    how: "cue 的目标和时间点是否清晰。这一项目前还没有自动评分。",
    basis: "一个能用的 cue 要有准备动作、明确目标、准确时间点，缺一样乐手就接不住。只剩一个指向的 cue 来得太晚，对方反应不过来。",
  },
  gesturePurpose: {
    label: "动作信息性",
    how: "每个动作是否都有明确目的。这一项目前还没有自动评分。",
    basis: "表情手越克制，它真正动起来的那一下越显眼。没有目的的持续晃动只是噪声，乐手会学着忽略你的表情手，等到真有事要说的时候反而没人看。",
  },
};
...
export const UNITS: { unit: 1 | 2 | 3; title: string; summary: string }[] = [
  {
    ...
    title: "拍子怎么打",
    summary:
      "打拍手的基本功。先把站姿和手的高度定下来，这个高度往后每一课都要用；再把 2、3、4 拍的图形拍型练到不用想下一拍往哪去；最后学速度快到逐拍打不过来时，怎么把一整小节归成一下。",
  },
  {
    unit: 2,
    title: "两只手分开用",
    summary:
      "让两只手分开工作。表情手停止镜像打拍手，转去做 cue、声部沟通和进出场提示。同一单元里还要学会乐曲从非第一拍开始时，预备拍该打在拍型的哪个位置。",
  },
  {
    unit: 3,
    title: "把音乐讲出来",
    summary:
      "用力度和速度的变化，把拍子变成音乐。拍型的尺寸表达音量，打拍的速度表达快慢，这两件事要在手上分得开。渐强时越打越快是最常见的混淆。",
  },
];
