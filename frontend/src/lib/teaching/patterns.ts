...
export const PATTERNS: Record<Meter, BeatPattern> = {
  ...
  2: {
    ...
    rebound: [
      { x: 0.52, y: 0.20 },   // 1→2：拍1向右甩再兜回
      { x: 0.46, y: 0.88 },   // 2→1：提拉后垂直下落
    ],
    mnemonic: ["下", "右上"],
    strokes: ["向右", "上提·预备"],
  },
  ...
  3: {
    ...
    rebound: [
      { x: 0.52, y: 0.12 },   // 1→2：拍1右甩再右弧
      { x: 0.80, y: 0.48 },   // 2→3：回拉缓冲再回中
      { x: 0.44, y: 0.94 },   // 3→1：左上延长再下落
    ],
    mnemonic: ["下", "右", "上"],
    strokes: ["向右", "回中", "上提·预备"],
  },
  ...
  4: {
    ...
    rebound: [
      { x: 0.48, y: 0.20 },   // 1→2：拍1向右拐+轻微上拉
      { x: 0.45, y: 0.35 },   // 2→3：向下缓冲再右扫
      { x: 0.76, y: 0.56 },   // 3→4：向下拉再上提到4
      { x: 0.50, y: 0.97 },   // 4→1：沿中线延长上提再回落
    ],
    mnemonic: ["下", "左", "右", "上"],
