 ...
 * 下→右→上，三条边都向内凹、三个拍点是尖角；4 拍下→左→右→上，横跨最宽，
 * 长扫弦会与落下的那一笔交叉）。
 *
 * ## 弧线往哪边弯，决定了拍点看不看得见
 *
 * 途经点（`via`）落在拍点连线的外侧还是内侧，是这三张图里唯一需要反复权衡的
 * 事情。外侧 = 弧线鼓出去，拍点被抹圆，整个图退化成一个圈；内侧 = 弧线凹进来，
 * 拍点成为尖角。三拍全部取内侧就是这个道理。二拍只有两个拍点，不存在内外之分，
 * 靠的是那个大圈本身；四拍的中间两笔要横跨最宽，取的是外侧。
 *
...
export const PATTERNS: Record<Meter, BeatPattern> = {
   ...
   * 三拍子：下 → 右 → 上，全程在身体中线偏右。
   *
   * **三条边都往里凹**，所以它是个凹三角形而不是一个鼓起来的圈。这不是画风问题：
   * 边鼓出去的时候三个拍点被弧线抹平，图上根本看不出拍打在哪；边凹进来，每个
   * 拍点就成了一个尖角 —— 而这张图存在的全部理由就是告诉人「拍在这三个位置」。
   * 顺带把它和二拍分开了：二拍是兜一个大圈，三拍是三个尖。
   */
  3: {
    meter: 3,
    ictus: [
