...
/**
 * 拍型 DTW 距离：≤0.14 满分，≥0.42 零分（归一化后的平均每点距离）。
 *
 * **这一维刻意判得松**：它要回答的是「乐手能不能看出你在打第几拍」，不是描红。
 * 手不可能和模板重合，而位置准不准、快慢稳不稳，另外四维已经分别在评了。
 *
 * 阈值是量出来的。一批「对但不标准」的样本 —— 真人抖动、拍点各挪 8~15%、反弹
 * 塌一半、整段越打越高 —— 距离落在 0.056~0.117；而唯一能被这套算法稳定认出来的
 * 错误「匀速划圈」在 0.365 以上。0.14 / 0.42 正好把这两堆分开。
 *
 * ## 它测不出什么（别指望，也别为此收窄阈值）
 *
 * 分不出「你打的其实是别的拍号」：该打二拍却打了三拍，距离只有 0.145，和一个
 * 拍点挪了 8% 的**正确**二拍（0.117）几乎重叠；四拍第 2 拍打反方向是 0.130，
 * 同样落在正确样本的范围里。原因见 `beatPattern.ts` 里 `dtwClassifier` 那段
 * ⚠️：归一化掉位置与大小之后各拍号的轮廓本来就极像，模板两两之间只差 0.034，
 * 真正的区别是**拍点落在这条线的哪几处**，而那份信息正好在归一化时被丢掉了。
 *
 * 所以收窄阈值换不回鉴别力 —— 旧的 0.08/0.30 下那几个真错误也只扣到 70~80 分，
 * 一样拦不住 —— 只会让打得对的人白白丢分。要诊断拍号打错，得把拍点时刻一起
 * 喂进来，那是另做一件事。
 */
const SHAPE_PERFECT = 0.14;
const SHAPE_ZERO = 0.42;
...
export function scoreSession(rec: Recording, opts: ScoreOptions): SessionScore {
  ...
  if (bars.length >= 1) {
    ...
    raw.set("patternShape", {
      ...
      // 这一维判得松（见 SHAPE_PERFECT 的说明），所以扣到分的基本只有
      // 「完全看不出拍型」那一类，措辞要对得上真正触发它的情形。
      advice:
        d <= SHAPE_PERFECT
          ? "拍型看得出来，够乐手辨认了。"
          : d < (SHAPE_PERFECT + SHAPE_ZERO) / 2
            ? "拍型大致对，但有几笔走偏了。对照示范看一遍每拍的走向顺序就行。"
            : "看不太出来在打第几拍 —— 多半是在匀速划圈。先照示范把每一拍的走向分清楚，宁可慢也别糊成一个圈。",
    });
  } else {
    raw.set("patternShape", {
      score: null,
