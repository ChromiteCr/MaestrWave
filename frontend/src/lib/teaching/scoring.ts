...
/**
 * 拍型 DTW 距离：≤0.10 满分，≥0.34 零分（归一化后的平均每点距离）。
 *
 * 比 M7j2 之前的 0.08/0.30 松一点点，但**刻意没有松到底**。松的理由：手不可能
 * 和模板重合，位置准不准、快慢稳不稳另外四维已经分别在评了，这一维只该回答
 * 「乐手能不能看出你在打第几拍」。不松到底的理由：松过头这一维就只剩「匀速划圈」
 * 一种错误还扣得下去，等于白占一份权重。
 *
 * 量出来的落点（真人抖动、拍点挪 8~15%、反弹塌一半、整段越打越高 = 打对了但
 * 不标准；后三行是真的打错了）：
 *
 * | 情形                    | 距离  | 0.08/0.30 | **0.10/0.34** |
 * |-------------------------|-------|-----------|---------------|
 * | 标准动作 / 真人抖动       | ≤0.09 |   96–100  |      100      |
 * | 拍点各挪 8%              | 0.117 |     83    |       93      |
 * | 整体倾斜（越打越高）       | 0.101 |     90    |       99      |
 * | 四拍第 2 拍打反方向        | 0.130 |     77    |       88      |
 * | 该打二拍却打了三拍         | 0.145 |     70    |       81      |
 * | 匀速划圈                 | ≥0.365|      0    |        0      |
 *
 * 打对了的人拿回该拿的分，打错的仍然扣得到 —— 但**只扣得动一点**，别指望更多：
 * 「打错了拍号」和「打对了但拍点挪了 8%」的距离本来就重叠（0.145 vs 0.117）。
 * 原因见 `beatPattern.ts` 里 `dtwClassifier` 那段 ⚠️：归一化掉位置与大小之后
 * 各拍号的轮廓极像，模板两两之间只差 0.034，真正的区别是**拍点落在这条线的
 * 哪几处**，而那份信息正好在归一化时被丢掉了。要真去诊断拍号打错，得把拍点
 * 时刻一起喂进 DTW，那是另做一件事。
 */
const SHAPE_PERFECT = 0.1;
const SHAPE_ZERO = 0.34;
